-- leaked by https://discord.gg/WfTDsBPR9n join for more op sources 
...
task.spawn(function()
    local function checkAndKick(gui)
    ...
    PlayerGui.DescendantAdded:Connect(checkAndKick)
end)
...
local rRainbowBase = CreateRow("Rainbow Base")
CreateToggleSwitch(rRainbowBase, Config.RainbowBase, function(ns, set)
    set(ns); Config.RainbowBase = ns; SaveConfig()
    ShowNotification("RAINBOW BASE", ns and "ENABLED" or "DISABLED")
end)
...
task.spawn(function()
    ...
    local function findMyPlotParts()
        local parts = {}
        local plots = Workspace:FindFirstChild("Plots")
    ...
    RunService.Heartbeat:Connect(function(dt)
        if not Config.RainbowBase then
            rainbowParts = {}
            return
        end
