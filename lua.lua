...
local DefaultConfig = {
    ...
    DuelBaseESP = true,
    AlertsEnabled = true,
    AlertSoundID = "rbxassetid://100173184074904",
...
local function ShowNotification(title, text)
    ...
    sg.Name = "XiNotif"; sg.ResetOnSpawn = false; sg.DisplayOrder = 999

    local f = Instance.new("Frame", sg)
    f.Size = UDim2.new(0, 300, 0, 62)
    f.Position = UDim2.new(1, -310, 1, -80)
    f.AnchorPoint = Vector2.new(0, 1)
    f.BackgroundColor3 = Color3.fromRGB(8, 10, 18)
    f.BackgroundTransparency = 1
    f.BorderSizePixel = 0
    Instance.new("UICorner", f).CornerRadius = UDim.new(0, 10)

    local stroke = Instance.new("UIStroke", f)
    stroke.Thickness = 1.2; stroke.Color = Theme.Accent2; stroke.Transparency = 1

    -- Barre coloree a gauche
    local bar = Instance.new("Frame", f)
    bar.Size = UDim2.new(0, 3, 1, -14); bar.Position = UDim2.new(0, 6, 0, 7)
    bar.BackgroundColor3 = Theme.Accent1; bar.BorderSizePixel = 0
    bar.BackgroundTransparency = 1
    Instance.new("UICorner", bar).CornerRadius = UDim.new(1, 0)

    -- Icone point lumineux
    local dot = Instance.new("Frame", f)
    dot.Size = UDim2.new(0, 7, 0, 7)
    dot.Position = UDim2.new(0, 16, 0, 13)
    ...
    t1.Size = UDim2.new(1, -36, 0, 20); t1.Position = UDim2.new(0, 30, 0, 8)
    ...
    t2.Size = UDim2.new(1, -36, 0, 18); t2.Position = UDim2.new(0, 30, 0, 30)
    ...
    progBg.BackgroundColor3 = Color3.fromRGB(30, 35, 50)
    ...
    progFill.BackgroundColor3 = Theme.Accent2
    ...
    TweenService:Create(dot,    fadeIn, {BackgroundTransparency = 0}):Play()
    TweenService:Create(t1,     fadeIn, {TextTransparency = 0}):Play()
    TweenService:Create(t2,     fadeIn, {TextTransparency = 0}):Play()
    TweenService:Create(progBg, fadeIn, {BackgroundTransparency = 0}):Play()

    -- Barre de progression qui se vide
    TweenService:Create(progFill, TweenInfo.new(2.2, Enum.EasingStyle.Linear), {Size = UDim2.new(0, 0, 1, 0)}):Play()

    task.delay(2.2, function()
        if not sg.Parent then return end
        local fadeOut = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In)
        TweenService:Create(f, fadeOut, {
...
task.spawn(function()
    ...
    local function updateUI(enabled, allPets)
        ...
        if SharedState.ListNeedsRedraw then
            ...
            if allPets and #allPets > 0 then
                for i = 1, #allPets do
                    ...
                    btn.Size = UDim2.new(1,0,0,44); btn.BackgroundColor3 = Theme.Surface
                    ...
                    -- Viewport a gauche
                    local vpFrame = Instance.new("Frame", btn)
                    vpFrame.Name = "BrainrotBg"
                    vpFrame.Size = UDim2.new(0, 38, 1, -6)
                    vpFrame.Position = UDim2.new(0, 8, 0, 3)
                    vpFrame.BackgroundColor3 = Color3.fromRGB(10, 10, 15)
                    vpFrame.BorderSizePixel = 0
                    vpFrame.ClipsDescendants = true
                    vpFrame.Visible = Config.ShowAnimalViewport
                    Instance.new("UICorner", vpFrame).CornerRadius = UDim.new(0, 5)

                    local vp = Instance.new("ViewportFrame")
                    vp.Name = "BrainrotViewport"
                    ...
                    task.spawn(pcall, function()
                        AnimalsShared:AttachOnViewport(
                            petData.animalData and petData.animalData.index or petData.petName,
                            vp,
                            nil,
                            petData.mutation ~= "None" and petData.mutation or nil
                        )
                    end)

                    local vpOffset = Config.ShowAnimalViewport and 50 or 10
                    local rankLabel = Instance.new("TextLabel", btn)
                    rankLabel.Size = UDim2.new(0, 24, 1, 0); rankLabel.Position = UDim2.new(0, vpOffset, 0, 0)
                    ...
                    infoLabel.Size = UDim2.new(1, -(vpOffset+26), 1, 0); infoLabel.Position = UDim2.new(0, vpOffset+26, 0, 0)
        ...
        listFrame.CanvasSize = UDim2.new(0,0,0, math.max(0, uiListLayout.AbsoluteContentSize.Y + 20))
    end
...
AddMobileMinimize(sFrame, "SpykeHub")
...
-- Barre de categories (scrollable horizontalement)
local catBar = Instance.new("ScrollingFrame", sFrame)
catBar.Size = UDim2.new(1, -20, 0, 32)
catBar.Position = UDim2.new(0, 10, 0, 44)
...
sList.Size = UDim2.new(1,-20,1,-86); sList.Position = UDim2.new(0,10,0,82)
...
local categories = {}
local activeCategoryFrame = nil
local activeCategoryBtn = nil
local currentCategoryName = nil

local function switchCategory(name)
    if currentCategoryName == name then return end
    currentCategoryName = name
    -- Cacher tous les frames
    for _, c in pairs(categories) do
        c.frame.Visible = false
        c.btn.BackgroundColor3 = Theme.SurfaceHighlight
        c.btn.TextColor3 = Theme.TextSecondary
    end
    -- Afficher la categorie active
    local cat = categories[name]
    if not cat then return end
    cat.frame.Visible = true
    cat.btn.BackgroundColor3 = Theme.Accent2
    cat.btn.TextColor3 = Color3.new(1,1,1)
    -- Mettre a jour le canvas
    sList.CanvasSize = UDim2.new(0,0,0, cat.layout.AbsoluteContentSize.Y + 20)
end

local function addCategory(name)
    if categories[name] then return end

    -- Bouton dans la barre
    local btn = Instance.new("TextButton", catBar)
    btn.Size = UDim2.new(0, math.max(60, #name * 7 + 16), 0, 26)
    btn.BackgroundColor3 = Theme.SurfaceHighlight
    btn.Text = name
    ...
    -- Frame de contenu (parent = sList)
    local frame = Instance.new("Frame", sList)
    ...
    categories[name] = {btn=btn, frame=frame, layout=layout}
    ...
    layout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        if currentCategoryName == name then
            sList.CanvasSize = UDim2.new(0,0,0, layout.AbsoluteContentSize.Y + 20)
        end
    end)

    -- Mettre a jour la largeur de la barre
    task.defer(function()
        catBar.CanvasSize = UDim2.new(0, catLayout.AbsoluteContentSize.X + 10, 0, 0)
    end)
end

-- Categorie courante pour CreateRow/CreateSectionHeader
local _currentCat = "GENERAL"

local function SetCategory(name)
    addCategory(name)
    _currentCat = name
    -- Activer la premiere categorie par defaut
    if not currentCategoryName then
        switchCategory(name)
    end
end
...
local function CreateRow(text, height)
    local cat = categories[_currentCat]
    local parent = cat and cat.frame or sList
...
local function CreateSectionHeader(text)
    ...
    local cat = catMap[text] or "MISC"
    SetCategory(cat)
end

-- Init categorie par defaut
SetCategory("TP")

local rTpOnLoad = CreateRow("Auto TP on Script Load")
CreateToggleSwitch(rTpOnLoad, Config.TpSettings.TpOnLoad, function(ns, set)
...
CreateSectionHeader("STEAL SPEED")
...
local rXrayTP = CreateRow("X-Ray")
CreateToggleSwitch(rXrayTP, Config.XrayEnabled, function(ns, set)
...
CreateToggleSwitch(rTimerESP, Config.TimerESP, function(ns, set)
    ...
    if ns then
        if _G.EnableTimerESP then _G.EnableTimerESP() end
    else
        if _G.DisableTimerESP then _G.DisableTimerESP() end
    end
...
local rShowViewport = CreateRow("Show Animal Viewport")
CreateToggleSwitch(rShowViewport, Config.ShowAnimalViewport, function(ns, set)
    set(ns); Config.ShowAnimalViewport = ns; SaveConfig()
    SharedState.ListNeedsRedraw = true
    if SharedState.UpdateAutoStealUI then SharedState.UpdateAutoStealUI() end
    ShowNotification("ANIMAL VIEWPORT", ns and "ENABLED" or "DISABLED")
end)
local rHideAutoBuy = CreateRow("Hide Auto Buy Panel")
CreateToggleSwitch(rHideAutoBuy, Config.HideAutoBuyUI, function(ns, set)
    set(ns); Config.HideAutoBuyUI = ns; SaveConfig()
    local abUI = PlayerGui:FindFirstChild("BullysAutoBuyUI")
    if abUI then
        local panel = abUI:FindFirstChild("ABPanel")
        if panel then panel.Visible = not ns end
    end
    ShowNotification("HIDE AUTO BUY", ns and "ENABLED" or "DISABLED")
end)
...
local rPlaySound = CreateRow("Test Alert Sound")
local bPlaySound = Instance.new("TextButton", rPlaySound)
bPlaySound.Size = UDim2.new(0, 90, 0, 24)
bPlaySound.Position = UDim2.new(1, -100, 0.5, -12)
bPlaySound.BackgroundColor3 = Theme.Accent2
bPlaySound.Text = "▶ PLAY"
bPlaySound.Font = Enum.Font.GothamBold
bPlaySound.TextColor3 = Color3.new(1,1,1)
bPlaySound.TextSize = 12
Instance.new("UICorner", bPlaySound).CornerRadius = UDim.new(0, 4)
bPlaySound.MouseButton1Click:Connect(function()
    local id = Config.AlertSoundID or ""
    if id == "" then ShowNotification("ALERT SOUND", "No sound ID set"); return end
    local s = Instance.new("Sound", Workspace)
    s.SoundId = id
    s.Volume = 0.5
    s:Play()
    game:GetService("Debris"):AddItem(s, 10)
    ShowNotification("ALERT SOUND", "Playing...")
end)
...
local rDesync = CreateRow("Desync Visualizer")
CreateToggleSwitch(rDesync, Config.DesyncVisualizer, function(ns, set)
    set(ns); Config.DesyncVisualizer = ns; SaveConfig()
    ShowNotification("DESYNC VISUALIZER", ns and "ENABLED" or "DISABLED")
end)
local function updateSettingsCanvasSize()
    if currentCategoryName and categories[currentCategoryName] then
        local layout = categories[currentCategoryName].layout
        sList.CanvasSize = UDim2.new(0, 0, 0, math.max(layout.AbsoluteContentSize.Y + 20, sList.AbsoluteSize.Y))
    end
end
...
do
    ...
    local function MakeKeybind(label, getVal, setVal, notifText)
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            local con; con = UserInputService.InputBegan:Connect(function(inp)
                if inp.UserInputType == Enum.UserInputType.Keyboard then
                    setVal(inp.KeyCode.Name)
                    btn.Text = inp.KeyCode.Name
                    btn.TextColor3 = Theme.TextPrimary
                    SaveConfig(); con:Disconnect()
                    ShowNotification(notifText, inp.KeyCode.Name)
                end
            end)
        end)
        return row
    ...
    MakeKeybind("Teleport",        function() return Config.TpSettings.TpKey end,        function(v) Config.TpSettings.TpKey = v end,        "TP KEYBIND")
    MakeKeybind("Clone",           function() return Config.TpSettings.CloneKey end,     function(v) Config.TpSettings.CloneKey = v end,     "CLONE KEYBIND")
    MakeKeybind("Carpet Speed",    function() return Config.TpSettings.CarpetSpeedKey end, function(v) Config.TpSettings.CarpetSpeedKey = v end, "CARPET SPEED KEYBIND")
    MakeKeybind("Steal Speed",     function() return Config.StealSpeedKey end,           function(v) Config.StealSpeedKey = v end,           "STEAL SPEED KEYBIND")
    MakeKeybind("Invisible Steal", function() return Config.InvisToggleKey end,          function(v) Config.InvisToggleKey = v; _G.INVISIBLE_STEAL_KEY = Enum.KeyCode[v] end, "INVIS KEYBIND")
    MakeKeybind("Reset",           function() return Config.ResetKey end,                function(v) Config.ResetKey = v end,                "RESET KEYBIND")
    MakeKeybind("Kick",            function() return Config.KickKey end,                 function(v) Config.KickKey = v end,                 "KICK KEYBIND")
    MakeKeybind("Ragdoll Self",    function() return Config.RagdollSelfKey end,          function(v) Config.RagdollSelfKey = v end,          "RAGDOLL SELF KEYBIND")
    MakeKeybind("Click To AP",     function() return Config.ClickToAPKeybind end,        function(v) Config.ClickToAPKeybind = v end,        "CLICK TO AP KEYBIND")
    MakeKeybind("Proximity AP",    function() return Config.ProximityAPKeybind end,      function(v) Config.ProximityAPKeybind = v end,      "PROXIMITY AP KEYBIND")
    MakeKeybind("Job Joiner",      function() return Config.JobJoinerKey end,            function(v) Config.JobJoinerKey = v end,            "JOB JOINER KEYBIND")
    MakeKeybind("Menu Toggle",     function() return Config.MenuKey end,                 function(v) Config.MenuKey = v end,                 "MENU KEYBIND")
    MakeKeybind("Desync",          function() return Config.DesyncKey end,               function(v) Config.DesyncKey = v end,               "DESYNC KEYBIND")
end
...
task.spawn(function()
    ...
    if privateBuild then
        ...
        author.Text = "discord.gg/spykehub"
    end
...
task.spawn(function()
    ...
    local spp = Instance.new("Part")
    spp.Name = "DesyncVisualizer"
    spp.Shape = Enum.PartType.Ball
    ...
    spp.Parent = Workspace

    local sAtt = Instance.new("Attachment", spp)
    local cAtt = Instance.new("Attachment")

    local beam = Instance.new("Beam")
    beam.Name = "DesyncBeam"
    ...
    RunService.Heartbeat:Connect(function()
        ...
        bb.Enabled = v
        spp.Transparency = v and 0.5 or 1
        beam.Enabled = v
...
task.spawn(function()
    ...
    local function createBaseESP(plot, mainPart)
        ...
        label.Font = Enum.Font.GothamBold
        label.TextColor3 = Theme.Accent1
        label.TextStrokeTransparency = 0
    ...
    _G.EnableTimerESP = enableTimerESP
    _G.DisableTimerESP = disableTimerESP
    ...
    task.spawn(function()
        local wasActive = Config.TimerESP
        while true do
            task.wait(0.5)
            if Config.TimerESP and not wasActive then
                wasActive = true
                enableTimerESP()
            elseif not Config.TimerESP and wasActive then
                wasActive = false
                disableTimerESP()
            end
        end
...
-- ── SPYKE BILLBOARD (tag au-dessus du joueur local uniquement) ───────────
task.spawn(function()
    local SPYKE_TAG = "Spyke"

    local function addSpykeTag(char)
        local hrp = char:FindFirstChild("HumanoidRootPart")
        if not hrp then return end

        local old = hrp:FindFirstChild("SpykeTag")
        ...
        lbl.TextSize = 14
        lbl.TextColor3 = Color3.fromRGB(0, 225, 255)
        lbl.TextStrokeColor3 = Color3.new(0, 0, 0)
        lbl.TextStrokeTransparency = 0
        lbl.Text = SPYKE_TAG .. " [" .. LocalPlayer.Name .. "]"
    end

    if LocalPlayer.Character then
        task.spawn(function() addSpykeTag(LocalPlayer.Character) end)
    end
...
-- ── AUTO DESYNC ON STEAL + DESYNC FREEZE POS ─────────────────────────────
do
    -- Position gelée côté serveur quand le desync est activé
    local _desyncFrozenPos = nil

    -- Expose la position gelée pour le visualizer
    _G.DesyncFrozenPos = nil

    -- Patch du visualizer pour utiliser la position gelée si dispo
    local _origDesyncHook = nil
    RunService.Heartbeat:Connect(function()
        if _G.DesyncFrozenPos then
            local spp = Workspace:FindFirstChild("DesyncVisualizer")
            if spp then
                spp.Position = _G.DesyncFrozenPos + Vector3.new(0, -2.5, 0)
            end
        end
    end)

    -- Quand on active le desync, on gèle la server pos à la position actuelle
    local _lastDesyncState = Config.DesyncVisualizer
    RunService.Heartbeat:Connect(function()
        local cur = Config.DesyncVisualizer
        if cur ~= _lastDesyncState then
            _lastDesyncState = cur
            if cur then
                -- On vient d'activer : geler la position
                local char = LocalPlayer.Character
                local hrp = char and char:FindFirstChild("HumanoidRootPart")
                if hrp then
                    _G.DesyncFrozenPos = hrp.Position
                end
            else
                _G.DesyncFrozenPos = nil
            end
    ...
    Config.AutoDesyncOnSteal = Config.AutoDesyncOnSteal or false

    LocalPlayer:GetAttributeChangedSignal("Stealing"):Connect(function()
        if not Config.AutoDesyncOnSteal then return end
        local state = LocalPlayer:GetAttribute("Stealing")
        if state then
            -- Début du steal : activer le desync et geler la pos
            if not Config.DesyncVisualizer then
                Config.DesyncVisualizer = true
                SaveConfig()
                local char = LocalPlayer.Character
                local hrp = char and char:FindFirstChild("HumanoidRootPart")
                if hrp then
                ...
                ShowNotification("AUTO DESYNC", "ACTIVÉ (steal détecté)")
            end
        else
            -- Fin du steal : désactiver le desync
            if Config.DesyncVisualizer then
                Config.DesyncVisualizer = false
                _G.DesyncFrozenPos = nil
                SaveConfig()
                ShowNotification("AUTO DESYNC", "DÉSACTIVÉ")
            end
        end
    end)

    -- UI : bouton toggle Auto Desync On Steal dans un petit panel
    task.spawn(function()
        local sg = Instance.new("ScreenGui")
        sg.Name = "DesyncControlUI"
        sg.ResetOnSpawn = false
        sg.DisplayOrder = 50
        sg.Parent = PlayerGui

        -- Bouton flottant pour ouvrir/fermer le panel
        local toggleBtn = Instance.new("TextButton", sg)
        toggleBtn.Size = UDim2.new(0, 120, 0, 28)
        toggleBtn.Position = UDim2.new(0.02, 0, 0.55, 0)
        toggleBtn.BackgroundColor3 = Theme.SurfaceHighlight
        toggleBtn.BorderSizePixel = 0
        toggleBtn.Font = Enum.Font.GothamBlack
        toggleBtn.TextSize = 11
        toggleBtn.TextColor3 = Theme.Accent1
        toggleBtn.Text = "DESYNC CONFIG +"
        toggleBtn.AutoButtonColor = false
        Instance.new("UICorner", toggleBtn).CornerRadius = UDim.new(0, 8)
        local tStroke = Instance.new("UIStroke", toggleBtn)
        tStroke.Color = Theme.Accent2; tStroke.Thickness = 1.5; tStroke.Transparency = 0.4
        MakeDraggable(toggleBtn, toggleBtn)

        local panel = Instance.new("Frame", sg)
        panel.Size = UDim2.new(0, 220, 0, 90)
        panel.Position = UDim2.new(0.02, 0, 0.55, 32)
        ...
        ps.Color = Theme.Accent2; ps.Thickness = 1.5; ps.Transparency = 0.4
        ...
        title.BackgroundTransparency = 1
        title.Text = "DESYNC CONFIG"
        title.Font = Enum.Font.GothamBlack
        title.TextSize = 12
        title.TextColor3 = Theme.Accent1
        title.TextXAlignment = Enum.TextXAlignment.Left

        -- Toggle Auto Desync On Steal
        local autoBtn = Instance.new("TextButton", panel)
        autoBtn.Size = UDim2.new(1, -16, 0, 26)
        autoBtn.Position = UDim2.new(0, 8, 0, 30)
        ...
        -- Toggle Desync Visualizer manuel
        local visBtn = Instance.new("TextButton", panel)
        visBtn.Size = UDim2.new(1, -16, 0, 26)
        visBtn.Position = UDim2.new(0, 8, 0, 60)
        visBtn.Font = Enum.Font.GothamBold
        visBtn.TextSize = 11
        visBtn.TextColor3 = Theme.TextPrimary
        visBtn.BorderSizePixel = 0
        Instance.new("UICorner", visBtn).CornerRadius = UDim.new(0, 6)

        local function refreshBtns()
            autoBtn.Text = Config.AutoDesyncOnSteal and "AUTO DESYNC ON STEAL: ON" or "AUTO DESYNC ON STEAL: OFF"
            autoBtn.BackgroundColor3 = Config.AutoDesyncOnSteal and Theme.Success or Theme.SurfaceHighlight

            visBtn.Text = Config.DesyncVisualizer and "DESYNC: ON" or "DESYNC: OFF"
            visBtn.BackgroundColor3 = Config.DesyncVisualizer and Theme.Accent1 or Theme.SurfaceHighlight
            visBtn.TextColor3 = Config.DesyncVisualizer and Color3.new(0, 0, 0) or Theme.TextPrimary

            toggleBtn.Text = panel.Visible and "DESYNC CONFIG -" or "DESYNC CONFIG +"
        end
        ...
        autoBtn.MouseButton1Click:Connect(function()
            Config.AutoDesyncOnSteal = not Config.AutoDesyncOnSteal
            SaveConfig()
            refreshBtns()
            ShowNotification("AUTO DESYNC ON STEAL", Config.AutoDesyncOnSteal and "ON" or "OFF")
        end)
        ...
        visBtn.MouseButton1Click:Connect(function()
            ...
            if Config.DesyncVisualizer then
                local char = LocalPlayer.Character
                local hrp = char and char:FindFirstChild("HumanoidRootPart")
                if hrp then _G.DesyncFrozenPos = hrp.Position end
            else
                _G.DesyncFrozenPos = nil
            end
            SaveConfig()
            refreshBtns()
            ShowNotification("DESYNC", Config.DesyncVisualizer and "ON" or "OFF")
        end)

        -- Sync visuel en temps réel
        RunService.Heartbeat:Connect(function()
            visBtn.BackgroundColor3 = Config.DesyncVisualizer and Theme.Accent1 or Theme.SurfaceHighlight
            visBtn.TextColor3 = Config.DesyncVisualizer and Color3.new(0, 0, 0) or Theme.TextPrimary
            visBtn.Text = Config.DesyncVisualizer and "DESYNC: ON" or "DESYNC: OFF"
...
task.spawn(function()
    ...
    local SharedConveyorAnimals = {}

    -- Détruire l'ancienne UI si elle existe
    ...
    local function buildCirclePresets()
        ...
        for i, td in ipairs(THEME_SWATCHES) do
            ...
            local match = cur
                and math.abs(cur.R - math.floor(col.R*255)) < 2
            ...
            cb.MouseButton1Click:Connect(function()
                Config.AutoBuyColor = {R=math.floor(col.R*255), G=math.floor(col.G*255), B=math.floor(col.B*255)}
                SaveConfig()
                local ring = Workspace:FindFirstChild("XiAutoBuyRing")
    ...
    local function toggleAutoBuy()
        ...
        ShowNotification("AUTO BUY", autoBuyActive and "✅ Activated" or "❌ Desactived")
    end
    ...
    task.spawn(function()
        ...
        local RARITY_WORDS = {common=true,uncommon=true,rare=true,epic=true,legendary=true,secret=true,divine=true,rainbow=true,cursed=true,gold=true,diamond=true}
        ...
        local function scanConveyor()
            ...
            for _, obj in ipairs(Workspace:GetDescendants()) do
                ...
                table.insert(results,{name=name, gen=gen, prompt=obj, part=realPart, model=model, source="ESTEIRA", uid="esteira_"..tostring(obj)})
            end
            return results
        end

        SharedConveyorAnimals = {}
        local function refreshConveyor()
            local ok, found = pcall(scanConveyor)
            if ok and found then SharedConveyorAnimals = found end
        end
        refreshConveyor()
        ...
        task.spawn(function()
            while true do
                ...
                if not autoBuyActive then
                    ...
                    stopCarpetLock(); destroyBodyPos()
                    continue
                end
                if lockedPart or lockedModel then
                    if not partAlive() then
                        ShowNotification("AUTO BUY","📦 Scanning...")
                        lockedTarget=nil; lockedPart=nil; lockedModel=nil
                    end
                    continue
                end
                ...
                for _,entry in ipairs(SharedConveyorAnimals) do
